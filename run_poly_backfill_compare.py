...
職責分工（兩階段，單一job內接力）：
  Phase 1（抓取）：讀 HF Dataset mp_data/ticker/ 現有票範圍 → 逐票抓 Polygon
    2年日線（end_date主動設為「昨天」，不抓「今天」——免費版對「今天」的
    range一律NOT_AUTHORIZED，範圍設定本身就避開這道牆，不需事後fallback，
    見DC tasks/v3_processor.py._fetch_polygon()同款根因但不同因應策略：
    DC是小範圍增量、被擋了用/prev補一天划算；這裡是2年大範圍，被擋等於
    整批作廢，主動避開才是對的）→ 寫 mp_data/ticker/{ticker}/d-p.csv
    （新檔名，不覆寫原本d.csv，兩份並存）
  Phase 2（核對）：找「d.csv 和 d-p.csv 都有、但還沒 compare_report.json」
    的票 → 比對OHLC數值差距（精確%，約略對照用）+ volume只比數量級（見下）
    + 精確日期缺口（MP有Poly沒有/反過來）→ 寫每票的 compare_report.json
    → 全部核對完後重建全域彙總 mp_poly_compare_summary.json（按差距大小排序）
...
停止條件（雙重，先到者為準）：MAX_JOB_SECONDS（5小時，GHA平台6小時硬頂
留緩衝）或 wall-clock 到 DAY_CUTOFF_HOUR（16:00 EST，收盤=盤後鏈接手的
時間點，之後絕不能還在跑Polygon抓取，見用戶2026-07-16拍板「盤後禁止」
規則）。斷點續傳：靠HF Dataset檔案存不存在（d-p.csv/compare_report.json）
...
之餘，平行額外dispatch本workflow（不是接在backtest後面——backtest同時被
DC晚上事件驅動觸發，接在它後面會導致Poly工作被牽連到晚上執行，違反
「盤後禁止」規則）。

回饋機制（用戶2026-07-16拍板「長時間工作要有充足回饋」，四層皆抄既有慣例）：
  1. Mongo心跳（同上，每20支）
  2. 熔斷：連續高比例失敗（CIRCUIT_MIN_SAMPLE起算，比例達CIRCUIT_FAIL_RATIO）
     → 提前中止本階段 + Telegram告警，比照DC VERIFY_BLOCK_MIN_SAMPLE/RATIO
  3. 自癒鎖：poly_backfill_lock，LOCK_STALE_SECONDS=6.5小時（比GHA 6小時
     硬頂多留緩衝），比照BC run_backtest_daily.py/BC.p db.py的鎖模式，
     真的卡死（如GHA平台強制砍job）不會永久卡住後續觸發
...
MAX_JOB_SECONDS    = 5 * 3600                 # 5小時預算
LOCK_STALE_SECONDS = MAX_JOB_SECONDS + 1.5 * 3600   # 6.5小時（登記於MongoDB_Standard）
DAY_CUTOFF_HOUR    = 16                       # EST 16:00 wall-clock 硬停
...
def main() -> int:
    ...
    if not db.acquire_lock():
        _log("⏭️ 搶不到鎖（上一輪還在跑，或未過 LOCK_STALE_SECONDS 判定孤兒），本次跳過")
        return 0

    try:
        # 停止條件 = min(5小時預算, wall-clock 16:00 EST) —— 先到者為準
        now = _now_est()
        cutoff_today = now.replace(hour=DAY_CUTOFF_HOUR, minute=0, second=0, microsecond=0)
        if now >= cutoff_today:
            _log(f"⏭️ 已過 {DAY_CUTOFF_HOUR}:00 EST，今日不再啟動新一輪")
            _notify(f"⏭️ [poly-backfill] 已過{DAY_CUTOFF_HOUR}:00 EST，本次跳過，等下次觸發")
            return 0

        wallclock_seconds_left = (cutoff_today - now).total_seconds()
        deadline_mono = start_mono + min(MAX_JOB_SECONDS, wallclock_seconds_left)

        _notify(
            f"🚀 [poly-backfill] 開始（本輪預算至多"
        ...
        compare_stats = {"processed": 0, "remaining_before": 0}
        summary = None
        if time.monotonic() < deadline_mono:
        ...
        # 兩階段都做完、時間還有剩 → 回頭繼續抓（用戶拍板「拼接成一條鏈」）
        if time.monotonic() < deadline_mono:
            fetch_stats2 = run_fetch_phase(db, api, deadline_mono)
            fetch_stats["processed"] += fetch_stats2["processed"]
            fetch_stats["failed"]    += fetch_stats2["failed"]

        msg = (
            f"📸 [poly-backfill] 本輪結束\n"
            f"抓取：本輪{fetch_stats['processed']}支（失敗{fetch_stats['failed']}）\n"
            f"核對：本輪{compare_stats['processed']}支\n"
        )
        if summary:
            msg += f"全域彙總：累計已核對{summary['ticker_count']}支（見 mp_poly_compare_summary.json）"
        _log(msg)
        _notify(msg)
