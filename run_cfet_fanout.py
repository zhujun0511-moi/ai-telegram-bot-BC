...
def main() -> int:
    ...
    url = f"https://api.github.com/repos/{cfet_repo}/actions/workflows/cfet_judge.yml/dispatches"
    try:
        resp = requests.post(
            url,
            json={"ref": "main"},
        ...
        print(f"[fanout] dispatch {cfet_repo}/cfet_judge.yml: {resp.status_code}")
        if resp.status_code >= 300:
            print(f"[fanout] response body: {resp.text[:500]}")
            return 1
        return 0
    except Exception as e:
        print(f"❌ [fanout] dispatch失敗: {e}")
        return 1
