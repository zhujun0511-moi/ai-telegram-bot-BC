...
鏈路：
  bc_refresh_models（本腳本，每日cron，唯一自動觸發源）
    ...
    → 若今天是平日（非週六週日）：額外 dispatch poly_backfill_compare.yml
      （2026-07-17新增，Poly長尾backfill+MP稽核，見該workflow/腳本檔頭
      docstring。刻意跟 bc_backtest_daily 平行、不是接在它後面——
      backtest同時被DC晚上事件驅動觸發，接在它後面會讓Poly抓取被牽連到
      晚上執行，違反「盤後禁止跑Polygon抓取」規則。只在平日dispatch是
      因為該工作本身鎖定「盤中idle時段」，週末沒有市場資料可抓，跑了
      也是空轉）
    → 若今天是週六或週日：額外 dispatch bc_verify_weekend.yml
      （沿用原本「週六本跑+週日兜底」的雙保險設計，verdict 冪等，
      重複觸發不會有副作用）

下游三個 workflow 保留各自的 workflow_dispatch 觸發（bc_backtest_daily
含既有的 chain_next 接力機制），只是拿掉了各自的 schedule（poly_backfill_
compare本來就沒有自己的schedule，一開始就是純workflow_dispatch設計）。
DC 事件驅動觸發 bc_backtest_daily 也是走 workflow_dispatch，不受影響。
...
def main() -> int:
    ...
    if is_weekend:
        ...
        _log("平日，額外 dispatch poly_backfill_compare.yml（Poly長尾backfill+MP稽核）")
        dispatch_workflow("poly_backfill_compare.yml")
